...
class BookingViewSet(viewsets.ModelViewSet):
    ...
    def available_coaches(self, request, pk=None):
        """
        Return all active staff members at this booking's location so the
        admin can pick a replacement coach.  Admin / superadmin only.
        """
    ...
    def check_coach_availability(self, request, pk=None):
        """
        Check whether a prospective new coach is available for the booking's
        existing time window.

        Query params:
          new_coach_id  – ID of the coach to check

        Returns:
          { "available": true }
          { "available": false, "conflict_type": "working_hours",    "message": "..." }
          { "available": false, "conflict_type": "booking_conflict", "message": "..." }
        ...
        if booking.booking_type != 'coaching':
            return Response(
                {'error': 'Only coaching bookings can have their coach changed.'},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        slot_start = booking.start_time   # UTC-aware
        slot_end   = booking.end_time     # UTC-aware
        ...
        # ── 1. Check booking conflict (coach already has another booking at this time) ──
        ...
        if booking_conflict:
            return Response({
                ...
                'message': (
                    f'{new_coach.first_name} {new_coach.last_name or ""}'.strip() +
                    ' already has a booking at this time. This cannot be overridden.'
                ),
            })
        ...
        # a) Full-day block?
        has_full_day_block = StaffBlockedDate.objects.filter(
            staff=new_coach,
            date=booking_date,
            start_time__isnull=True,
            end_time__isnull=True,
        ...
        if has_full_day_block:
            return Response({
                ...
                'message': (
                    f'{new_coach.first_name} {new_coach.last_name or ""}'.strip() +
                    ' has a full-day block on this date (outside scheduled working hours).'
                ),
            })

        # b) Partial-day blocks that overlap the slot?
        partial_blocks = StaffBlockedDate.objects.filter(
            staff=new_coach,
            date=booking_date,
            start_time__isnull=False,
        ...
        for b in partial_blocks:
            ...
            if slot_start < b_end_utc and slot_end > b_start_utc:
                coach_name = f'{new_coach.first_name} {new_coach.last_name or ""}'.strip()
                return Response({
                    'available': False,
                    'conflict_type': 'working_hours',
                    'message': (
                        f'{coach_name} is blocked during part of this time slot '
                        '(outside scheduled working hours). You can force override.'
                    ),
                })

        # c) Does a shift window cover the full slot?
        #    Prefer specific-date override, fall back to recurring weekly.
        availabilities = list(StaffDayAvailability.objects.filter(
            staff=new_coach, date=booking_date
        ).values('start_time', 'end_time'))

        if not availabilities:
            availabilities = list(StaffAvailability.objects.filter(
                staff=new_coach, day_of_week=day_of_week
            ).values('start_time', 'end_time'))
        ...
        if not availabilities:
            ...
            return Response({
                ...
                'conflict_type': 'working_hours',
                'message': (
                    f'{coach_name} has no scheduled working hours on this day. '
                    'You can force override.'
                ),
            })

        # Convert availability windows to UTC and check coverage
        is_covered = False
        ...
        if not is_covered:
            ...
            return Response({
                ...
                'conflict_type': 'working_hours',
                'message': (
                    f'{coach_name} is not available during this time slot '
                    '(outside scheduled working hours). You can force override.'
                ),
            })
    ...
    def change_coach(self, request, pk=None):
        """
        Reassign the coach on a coaching booking.

        Payload:
          { "new_coach_id": <int>, "force_override": <bool> }

        Rules:
          - Admin / superadmin only.
          - booking_conflict  → never overridable (returns 400 even with force_override).
          - working_hours     → overridable when force_override=true.
        """
        if not (request.user.is_superuser or getattr(request.user, 'role', '') in ('admin', 'superadmin')):
            return Response({'error': 'Permission denied.'}, status=status.HTTP_403_FORBIDDEN)
        ...
        if booking.status in ('cancelled', 'completed'):
            return Response(
                {'error': f'Cannot change coach on a {booking.status} booking.'},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if booking_conflict:
            # Booking conflicts are NEVER overridable
            return Response(
                {
                    'error': (
                        f'{new_coach.first_name} {new_coach.last_name or ""}'.strip() +
                        ' already has a booking at this time. Cannot override a booking conflict.'
                    ),
                    'conflict_type': 'booking_conflict',
                },
                status=status.HTTP_400_BAD_REQUEST,
        ...
        # If not force-overriding, validate working hours as well
        if not force_override:
            ...
            # Full-day block
            if StaffBlockedDate.objects.filter(
                staff=new_coach, date=booking_date, start_time__isnull=True, end_time__isnull=True
            ).exists():
                return Response(
                    {
                        'error': (
                            f'{new_coach.first_name} {new_coach.last_name or ""}'.strip() +
                            ' has a full-day block on this date.'
                        ),
                        'conflict_type': 'working_hours',
                    },
                    status=status.HTTP_400_BAD_REQUEST,
            ...
            # Partial blocks overlapping slot
            for b in StaffBlockedDate.objects.filter(
                staff=new_coach, date=booking_date,
                    ...
                    return Response(
                        {
                            'error': (
                                f'{new_coach.first_name} {new_coach.last_name or ""}'.strip() +
                                ' is blocked during this time slot. Use force override.'
                            ),
                            'conflict_type': 'working_hours',
                        },
                        status=status.HTTP_400_BAD_REQUEST,
                ...
                return Response(
                    {
                        'error': (
                            f'{new_coach.first_name} {new_coach.last_name or ""}'.strip() +
                            ' has no scheduled working hours on this day. Use force override.'
                        ),
                        'conflict_type': 'working_hours',
                    },
                    status=status.HTTP_400_BAD_REQUEST,
                ...
                return Response(
                    {
                        'error': (
                            f'{new_coach.first_name} {new_coach.last_name or ""}'.strip() +
                            ' is outside their scheduled working hours for this slot. Use force override.'
                        ),
                        'conflict_type': 'working_hours',
                    },
                    status=status.HTTP_400_BAD_REQUEST,
        ...
        # ── Apply the change ──────────────────────────────────────────────────
        old_coach_name = (
            f'{booking.coach.first_name} {booking.coach.last_name or ""}'.strip()
            if booking.coach else 'Unassigned'
        )
        new_coach_name = f'{new_coach.first_name} {new_coach.last_name or ""}'.strip()

        booking.coach = new_coach
        booking.save(update_fields=['coach', 'updated_at'])

        logger.info(
            'Booking #%s coach changed from "%s" to "%s" by %s (force_override=%s)',
            booking.id, old_coach_name, new_coach_name, request.user, force_override,
        )

        serializer = self.get_serializer(booking)
        return Response({
            'message': f'Coach changed from {old_coach_name} to {new_coach_name}.',
            'force_overridden': force_override,
            'booking': serializer.data,
        })
