...
def coupon_applicable_at_location(coupon, location_id) -> bool:
    """True if coupon is global or scoped to this GHL location (matches validate queryset)."""
    scoped = (getattr(coupon, 'location_id', None) or '').strip()
    if not scoped:
        return True
    loc = (location_id or '').strip()
    return bool(loc) and scoped == loc
