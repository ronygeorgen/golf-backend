...
def send_payment_link_email(
    *,
    ...
    Includes base / discount / tax / total breakdown when amounts are provided.
    ...
    try:
        ...
        if tax_rate is not None:
            try:
                ...
                rate = None
        if rate is None and ghl_location and getattr(ghl_location, 'tax_rate', None) is not None:
            try:
                rate = float(ghl_location.tax_rate)
            except (TypeError, ValueError):
                rate = None
        if rate is None:
            rate = 0.14
        ...
        if base is not None:
            ...
            if tax is not None:
                tax_row = f'''
            <tr>
              <td style="padding:10px 0;border-bottom:1px solid #eee;color:#555;">{tax_label}</td>
    ...
    try:
        ...
        def _fmt_block_window():
            if block_date and block_start and block_end:
                ...
                return f"{day}, {start_t}  {end_t}"
            if block_date:
